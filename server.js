...
function analyzeBloomsLevel(question) {
  const rememberKeywords = ['what', 'who', 'when', 'where', 'list', 'define'];
  const understandKeywords = ['explain', 'describe', 'summarize', 'interpret'];
  const applyKeywords = ['solve', 'calculate', 'demonstrate', 'use'];
  const analyzeKeywords = ['analyze', 'compare', 'contrast', 'examine'];
  const evaluateKeywords = ['evaluate', 'assess', 'judge', 'critique'];
  const createKeywords = ['create', 'design', 'build', 'construct'];

  const lowerQuestion = question.toLowerCase();
  
  if (createKeywords.some(kw => lowerQuestion.includes(kw))) return 'create';
  if (evaluateKeywords.some(kw => lowerQuestion.includes(kw))) return 'evaluate';
  if (analyzeKeywords.some(kw => lowerQuestion.includes(kw))) return 'analyze';
  if (applyKeywords.some(kw => lowerQuestion.includes(kw))) return 'apply';
  if (understandKeywords.some(kw => lowerQuestion.includes(kw))) return 'understand';
  if (rememberKeywords.some(kw => lowerQuestion.includes(kw))) return 'remember';
...
function createServer(port) {
  const server = http.createServer(async (req, res) => {
    ...
    if (method === 'POST' && pathName === '/ask') {
      ...
      req.on('end', async () => {
        try {
          const { question, mode = 'normal', subject } = JSON.parse(body);
          ...
          if (datasetAnswer && confidence > 70) {
            ...
            try {
              answer = await queryGroq(trimmedQuestion, userData?.preferences || {}, pdfContext, mode);
